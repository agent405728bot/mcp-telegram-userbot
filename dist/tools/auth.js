...
export function registerAuthTools(server, telegram) {
    ...
    server.registerTool("telegram-login", {
        description: "Login to Telegram via QR code. Returns QR image, ASCII QR, and the login URL. IMPORTANT: pass the entire result to user without modifications.",
        annotations: WRITE,
    }, async () => {
        let qrDataUrl = "";
        let qrUrl = "";
        ...
        // Generate ASCII QR for terminal clients
        let asciiQr = "";
        try {
            asciiQr = await QRCode.toString(qrUrl || "placeholder", {
                type: "terminal",
                small: true,
            });
        } catch {
            asciiQr = "(ASCII QR generation failed)";
        }

        const instructions = [
            "📱 Scan this QR code in Telegram: **Settings → Devices → Link Desktop Device**.",
            "",
            "🔗 **Login URL** (copy-paste if QR doesn't work):",
            "```",
            qrUrl,
            "```",
            "",
            "🖼️ **QR Image file**: " + qrFilePath,
            "",
            "⌨️ **ASCII QR** (for terminal clients):",
            "```",
            asciiQr,
            "```",
            "",
